...
int64_t PerTileCacheLines(const TiledHloInstruction& inst) {
  ...
  int64_t minor_dim = LayoutUtil::Minor(shape.layout(), 0);
  int64_t tile_minor_size = llvm::PowerOf2Ceil(inst.tile_size(minor_dim));
  ...
  for (auto [dim, size] : llvm::enumerate(inst.tile_sizes())) {
    if (dim != minor_dim) {
