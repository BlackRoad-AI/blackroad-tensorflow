...
XLA performs checks to ensure that the aggregate size of all necessary
allocations fits in the device's HBM.
...
usage Y"*. → Jump to
[Section 1: TC + SC HBM usage Exceeds Limit](#balance-sc-tc-usage).
...
[Section 2: Unexpectedly Large Allocations](#large-buffers).
...
[Section 3: Aggregate Allocations Exceed HBM Limit](#aggregate-oom).

---

### Section 1. Balance TC + SC Usage {#balance-sc-tc-usage}
...
* **High TensorCore Usage:**
  * Proceed to [Section 3](#aggregate-oom).
* **Balanced**
...
recommendations in this section and [Section 3](#aggregate-oom).
...
* **Remove Debugging Artifacts:**
  * Using [jax.debug.print()](https://docs.jax.dev/en/latest/_autosummary/jax.debug.print.html)
  ...
  usage.
  * Check for any left-over `jax.debug.print()`s and remove them.
...
XLA::Rematerialization pass to prioritize memory savings, potentially at the cost
of slower compilations:
